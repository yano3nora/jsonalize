...
function parseArgs(): {
  ...
  const version = rawVersion ?? Deno.env.get('JSONALIZE_RELEASE_VERSION')
...
async function bumpVersion(version: string): Promise<void> {
  ...
  const versionPattern = /export const VERSION = 'jsonalize (\d+\.\d+\.\d+)'/
  ...
  if (currentVersion === version) {
    console.log(
      `VERSION is already jsonalize ${version}; leaving main.ts unchanged.`,
    )
    return
  }
  ...
  const next = source.replace(
    versionPattern,
    `export const VERSION = 'jsonalize ${version}'`,
  )
...
async function assertCliVersion(version: string): Promise<void> {
  ...
  const expected = `jsonalize ${version}`
...
// compile 成功だけでは実行時の不具合を検出できないため、
...
async function smokeTestCompiledBinary(version: string): Promise<void> {
  ...
  try {
    const versionOut = (await run('./jsonalize', ['--version'])).trim()
    if (versionOut !== `jsonalize ${version}`) {
    ...
    if (!smokeOut.includes('"id"')) {
      throw new Error('Compiled binary smoke test failed: no JSON output.')
    }
  } finally {
    await Deno.remove('./jsonalize')
  }
}
